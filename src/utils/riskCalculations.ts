...
export function calculateDerivedMetrics(logs: { weights: WeightLog[], bps: BPLog[], foods: FoodLog[], exercises: ExerciseLog[] }) {
  const now = new Date('2026-03-02T13:39:44-08:00').getTime();
  ...
  const recentExercises = logs.exercises.filter(e => new Date(e.timestamp).getTime() >= exercise7DaysAgo);
  ...
  const recentBPs = logs.bps.filter(bp => new Date(bp.timestamp).getTime() >= bp14DaysAgo);
  let avgSBP = 0;
  let avgDBP = 0;
  if (recentBPs.length > 0) {
    avgSBP = recentBPs.reduce((sum, bp) => sum + bp.systolic, 0) / recentBPs.length;
    avgDBP = recentBPs.reduce((sum, bp) => sum + bp.diastolic, 0) / recentBPs.length;
  }

  // 4. Calories Average (past 7 days)
  const food7DaysAgo = now - daysInMs(7);
  const recentFoods = logs.foods.filter(f => new Date(f.timestamp).getTime() >= food7DaysAgo);
  const totalCalories7Days = recentFoods.reduce((sum, f) => sum + f.calories, 0);
...
export function calculateRiskScores(userData: any, derivedMetrics: any) {
  const { age, gender, height, weight, waist, systolicBP, diastolicBP, fastingGlucose } = userData;
  ...
  // Weight Trend norm
  const norm_Wt = Math.min(1, Math.max(0, weightSlope) / 5);

  // Activity norm
  const norm_Act = Math.max(0, Math.min(1, 1 - (act7 / 150)));

  // Diabetes Risk
  const diabetesRisk = 100 * (0.30 * norm_G + 0.25 * norm_B + 0.20 * norm_W + 0.15 * norm_Wt + 0.10 * norm_Act);
  ...
  // Hypertension Risk
  const hypertensionRisk = 100 * (0.50 * norm_BP + 0.20 * norm_B + 0.15 * norm_Wt + 0.15 * norm_Act);
